...
(function () {
    ...
    const ADDRESS_LOCATION_CACHE_KEY = "gensaude_localizacao_endereco";
    ...
    const SEARCH_RADII_METERS = [5000, 10000, 20000];
    ...
    const UNIDADES_OFICIAIS_TABOAO = [
        {
            id: "oficial-taboao-ubs-margaridas",
        ...
        {
            id: "oficial-taboao-ubs-parque-pinheiros",
        ...
        {
            id: "oficial-taboao-ubs-maria-jose",
        ...
        {
            id: "oficial-taboao-ubs-oliveiras",
        ...
        {
            id: "oficial-taboao-ubs-santo-onofre",
        ...
        {
            id: "oficial-taboao-ubs-silvio-sampaio",
        ...
        {
            id: "oficial-taboao-ubs-santa-cecilia",
        ...
        {
            id: "oficial-taboao-ubs-panorama",
            nome: "UBS Panorama",
            tipo: "ubs",
        ...
        {
            id: "oficial-taboao-ubs-tania-regina",
        ...
        {
            id: "oficial-taboao-ubs-clementino",
        ...
        {
            id: "oficial-taboao-upa-akira-tada",
    ...
    async function localizarPorCepOuDispositivo() {
        ...
        if (endereco) {
            ...
            try {
                const localizacao =
                    await buscarCoordenadasDoEndereco(
                        endereco
                    );
                ...
                await processarLocalizacao(
                    ...
                    50,
                    "Endereço informado na avaliação"
                );

                return;
            } catch (erro) {
                console.warn(
                    "Não foi possível localizar o endereço completo. Tentando o CEP:",
                    erro
                );
            }
        }
    ...
    function obterEnderecoDaAvaliacao() {
        const dados =
    ...
    async function buscarCoordenadasDoEndereco(
        endereco
    ) {
        const cache =
            carregarCacheDoEndereco();
        ...
        if (!resposta.ok) {
            throw new Error(
                `A consulta do endereço retornou ${resposta.status}.`
            );
        }
        ...
        if (!primeiroResultado) {
            throw new Error(
                "O endereço completo não foi encontrado."
            );
        }

        const latitude =
            Number(
                primeiroResultado.lat
            );

        const longitude =
            Number(
                primeiroResultado.lon
            );
        ...
        if (
            ...
            throw new Error(
                "O endereço não possui coordenadas válidas."
            );
        }
    ...
    function carregarCacheDoEndereco() {
        const dados =
    ...
    async function processarLocalizacao(
        ...
        try {
            const unidadesOficiais =
                obterUnidadesOficiaisDaCidade();

            let unidades =
                unidadesOficiais;

            /*
             * Em Taboão da Serra já temos UBSs e UPA
             * confirmadas no próprio projeto.
             *
             * Nesse caso não aguardamos o Overpass.
             */
            if (
    ...
    // =====================================================
    // UNIDADES OFICIAIS DA CIDADE
    // =====================================================

    function obterUnidadesOficiaisDaCidade() {
        const dados =
            avaliacao.dadosPessoais ||
            {};
        ...
        return UNIDADES_OFICIAIS_TABOAO
            .map(
                (unidade) => ({
                    ...unidade
                })
    ...
    async function buscarUnidadesDeSaude(
        ...
        let ultimoErro = null;
        let recebeuRespostaValida = false;
        ...
        return `
            ...
            (
                nwr(around:${raio},${latitude},${longitude})
                    ["name"~"Unidade Básica|Unidade Basica|Posto de Saúde|Posto de Saude|Centro de Saúde|Centro de Saude|Pronto Atendimento|Pronto Socorro",i];

                nwr(around:${raio},${latitude},${longitude})
                    ["name"~"(^|[^A-Za-z])(UBS|USF|ESF|UPA|AMA)([^A-Za-z]|$)",i];

                nwr(around:${raio},${latitude},${longitude})
                    ["short_name"~"^(UBS|USF|ESF|UPA|AMA)( |$)",i];

                nwr(around:${raio},${latitude},${longitude})
                    ["amenity"~"clinic|doctors|health_post"];

                nwr(around:${raio},${latitude},${longitude})
                    ["healthcare"~"clinic|doctor|centre|health_post"];
    ...
    // =====================================================
    // SELECIONAR UNIDADES MAIS PRÓXIMAS
    // =====================================================
    ...
    function mostrarCarregamentoDasUnidades() {
        elements.unitsList.innerHTML = `
            <div class="unit-loading-placeholder">
                <span class="unit-loading-line unit-loading-title"></span>
                <span class="unit-loading-line unit-loading-distance"></span>
            </div>
    ...
    function refazerAvaliacao() {
         ...
         * A conta e a sessão continuam no localStorage.
         */
